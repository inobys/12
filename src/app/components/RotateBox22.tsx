...
const RotateCol = memo(({ list, index, col, duration, callback, onRef }) => {
    ...
    const goodsDom = list.map((g, q) => {
        ...
        return (<div className={`rotate-goods ${(g.sp && g.sp == 1) ? ((g.xx && g.xx == 1) ? 'x xx' : 'x') : ''} ${g.picNotHaveBg ? g.picNotHaveBg : ''}`} ref={itemRefInstance}>
...
const RotateBox = memo(({ result, isPlay = false, onPLay = () => { }, callback }) => {
    ...
    const type = result.length == 10 ? 2 : 1; // 修改判断条件，10个奖品为十抽
    let disableArray = Array(10).fill(false); // 修改为10个位置
    if (type == 1) {
        disableArray = Array(10).fill(true); // 修改为10个位置
        disableArray[2] = false; // 单抽时只激活中间位置
    } else {
        // 十抽时激活所有10个位置
        disableArray = Array(10).fill(false);
    }
    let rotateCallCount = 0;
    let rotateCallBack = () => {
        if (++rotateCallCount >= (type == 1 ? 1 : 10)) { // 修改为10次回调
            // setTimeout(() => {
    ...
    const listCol = Array(10).fill(list); // 修改为10列

    //孤胆玩家柯尔特  无畏玩家雪莉 热血玩家柯尔特  爆裂玩家雪莉  霓虹骇客莫提斯  旧：5848221,5848220,5848219,5848218,5848257
    const listDefault = [
        6643185, 6643186, 6643187, 6643188, 6643190, // 第 1 行
        6643191, 6643192, 6643193, 6643194, 6643195  // 第 2 行
    ];
    const listDom = listCol.map((col, idx) => {
        ...
        if (isPlay && result.length) {
            ...
            if (res.iPackageId == defaultId) {
                defaultId = 6643185;//当抽中默认展示的道具时，默认展示的道具icon改成镖客柯尔特  旧：恶女艾魅【5848223】      update by fqf 202506
            }
        ...
        return (<RotateCol list={rList} // 移除重复的列表
            index={4}
            col={idx + 1}
            callback={rotateCallBack}
            onRef={(ref) => {
                rotateColRef[idx].current = ref;
                console.log(rotateColRef[idx].current,'11111111111111111111111111111')
